...
class _RegistrarKilosScreenState extends State<RegistrarKilosScreen> {
  ...
  Future<void> _cargarTrabajadoresPorLote() async {
    ...
    try {
      ...
      if (selectedFarmId != null) {
        final selectedFarm = fincas.firstWhere(
  ...
  Future<void> _cargarRecolecciones() async {
    ...
    try {
      final recoleccionesList = await _recoleccionService
          .getRecoleccionesByFarm(selectedFarmId!);
      setState(() {
                  ...
                  if (nombre.isNotEmpty) {
                    ...
                    if (trabajadorExiste) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Trabajador "$nombre" agregado correctamente',
                          ),
                          backgroundColor: Colors.green,
                        ),
                      );
  ...
  Future<void> _guardarTrabajadorGeneral(String nombre) async {
    try {
      await FirebaseFirestore.instance.collection('trabajadores_lote').add({
        'nombre': nombre,
        'loteld':
            'general', // Lo guardamos como general para que esté disponible en todos los lotes
        'farmId': selectedFarmId,
        'timestamp': FieldValue.serverTimestamp(),
      });
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error al guardar trabajador: $e')));
    }
  }
  ...
  void _mostrarDialogoRecoleccion() {
    ...
    String loteld =
        selectedFarm.plots.isNotEmpty ? selectedFarm.plots.first.name : '';
    String trabajador =
        trabajadoresPorLote[loteld]?.isNotEmpty == true
                ...
                (context, setState) => AlertDialog(
                    ...
                    children: [
                      ...
                      if (loteld.isNotEmpty)
                        DropdownButtonFormField<String>(
                          ...
                          items:
                              trabajadoresPorLote[loteld]?.map((t) {
                                return DropdownMenuItem<String>(
                                  value: t,
                                  child: Text(t),
                                );
                          ...
                          onChanged:
                              (value) => setState(() => trabajador = value ?? ''),
                        ),
                      const SizedBox(height: 10),
                      InkWell(
                        onTap: () {
                  ...
                  actions: [
                    ...
                    TextButton(
                      onPressed: () async {
                        if (loteld.isNotEmpty &&
                            trabajador.isNotEmpty &&
                            kilosController.text.isNotEmpty) {
                          await _guardarRegistroKilos(
                            fecha,
                            loteld,
                            trabajador,
                            kilosController.text,
  ...
  Future<void> _guardarRegistroKilos(
    String fecha,
    String loteld,
    String trabajador,
    String kilos,
    ...
    try {
      ...
      final recoleccion = Recoleccion(
        ...
        data: dataActualizado,
        timestamp:
            existingRecoleccion.id.isEmpty
                ? Timestamp.now()
  ...
  // Método para calcular total del trabajador en el mes actual
  double _getTotalMensualTrabajador(String trabajador) {
    ...
    for (var recoleccion in recolecciones) {
      final fechaRecoleccion = DateFormat('yyyy-MM-dd').parse(recoleccion.fecha);

      // Solo contar si está en el mes actual
      if (fechaRecoleccion.isAfter(inicioMes.subtract(const Duration(days: 1))) &&
          fechaRecoleccion.isBefore(finMes.add(const Duration(days: 1)))) {
        final datosTrabajador = recoleccion.data[trabajador];
        if (datosTrabajador != null && datosTrabajador['kilos'] != null) {
          final kilos = double.tryParse(datosTrabajador['kilos'].toString()) ?? 0;
          total += kilos;
        }
      }
    }
  ...
  // Método para obtener historial del trabajador
  List<Map<String, dynamic>> _getHistorialTrabajador(String trabajador) {
  ...
  // Widget para mostrar el historial completo
  Widget _buildHistorialView() {
    // Obtener todos los trabajadores únicos
    Set<String> todosLosTrabajadores = {};
    for (var recoleccion in recolecciones) {
      todosLosTrabajadores.addAll(recoleccion.data.keys);
    }
    ...
    return ListView.builder(
      ...
      itemCount: todosLosTrabajadores.length,
      itemBuilder: (context, index) {
        final trabajador = todosLosTrabajadores.elementAt(index);
        final historial = _getHistorialTrabajador(trabajador);
        final totalMensual = _getTotalMensualTrabajador(trabajador);
        ...
        return Card(
          ...
          child: ExpansionTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text(
                  'Total del mes: ${totalMensual.toStringAsFixed(1)} kg',
                  style: TextStyle(
                    color: Colors.green[700],
              ...
              if (historial.isEmpty)
                ...
                Column(
                  children: [
                    ...
                    // Mostrar los registros (máximo 10)
                    ...historial.take(10).map((registro) {
                      final fecha = DateFormat(
                        'dd/MM/yyyy',
                      ).format(DateFormat('yyyy-MM-dd').parse(registro['fecha']));
                    ...
                    if (historial.length > 10)
                      Padding(
                        padding: const EdgeInsets.all(16.0),
                        child: Text(
                          'Y ${historial.length - 10} registros más...',
                          style: TextStyle(
  ...
  // Método para calcular total de un lote en una fecha específica
  ...
  // Widget para mostrar registro actual
  Widget _buildRegistroActualView() {
    final registroActual = _getRegistroActual();

    if (registroActual.isEmpty) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            SizedBox(height: 16),
            Text(
              'No hay recolecciones registradas hoy',
              style: TextStyle(fontSize: 16, color: Colors.grey),
            ),
          ],
        ),
    ...
    return ListView.builder(
      padding: const EdgeInsets.all(16.0),
      itemCount: registroActual.length,
      itemBuilder: (context, index) {
        final recoleccion = registroActual[index];
        ...
        return Card(
          ...
          child: ExpansionTile(
            title: Text(
              '${recoleccion.loteld} - ${recoleccion.fecha} ($diaSemana)',
              style: const TextStyle(fontWeight: FontWeight.w600),
            ),
            subtitle: Text(
              'Total: ${total.toStringAsFixed(1)} kg',
              style: TextStyle(
                color: Colors.green[700],
            ...
            children:
                (trabajadoresPorLote[recoleccion.loteld] ?? [])
                    .where(
                      (t) => t
                          .toLowerCase()
                          .contains(
                    ...
                    .map((trabajador) {
                      final kilos =
                          recoleccion.data[trabajador]?['kilos'] ?? '0';
                      final kilosNum = double.tryParse(kilos.toString()) ?? 0;
                      
                      // Solo mostrar trabajadores que tienen kilos registrados
                      if (kilosNum > 0) {
                        return ListTile(
                          leading: CircleAvatar(
